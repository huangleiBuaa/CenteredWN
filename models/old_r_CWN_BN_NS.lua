...
local function createModel(opt)
   ...
   local function basicblock(n, stride)
    ...
    --  s:add(Convolution(nInputPlane,n,3,3,stride,stride,1,1))
       s:add(cudnn.Spatial_Weight_CenteredBN(nInputPlane,n,3,3,stride,stride,1,1):noBias())
     --  s:add(nn.Spatial_Scaling(n,opt.BNScale,false))

      s:add(SBatchNorm(n))
      s:add(ReLU(true))
     -- s:add(Convolution(n,n,3,3,1,1,1,1))
      s:add(cudnn.Spatial_Weight_CenteredBN(n,n,3,3,1,1,1,1):noBias())
     --  s:add(nn.Spatial_Scaling(n,opt.BNScale,false))
   ...
   local model = nn.Sequential()
     ...
     -- model:add(Convolution(3,16,3,3,1,1,1,1))
      model:add(cudnn.Spatial_Weight_CenteredBN(3,16,3,3,1,1,1,1):noBias())
   ...
   ConvInit('cudnn.Spatial_Weight_CenteredBN')
