...
require 'cunn'
require 'cudnn'
require '../../../NNNetwork/module/spatial/cudnn_Spatial_Weight_CenteredBN'

local function inception(input_size, config)
   local concat = nn.Concat(2)
   if config[1][1] ~= 0 then
      local conv1 = nn.Sequential()
      conv1:add(cudnn.Spatial_Weight_CenteredBN(input_size, config[1][1],1,1,1,1)):add(cudnn.ReLU(true))
   ...
   conv3:add(cudnn.Spatial_Weight_CenteredBN(  input_size, config[2][1],1,1,1,1)):add(cudnn.ReLU(true))
   conv3:add(cudnn.SpatialBatchNormalization(config[2][1],1e-3))
   conv3:add(cudnn.ReLU(true))
   conv3:add(cudnn.Spatial_Weight_CenteredBN(config[2][1], config[2][2],3,3,1,1,1,1)):add(cudnn.ReLU(true))
   conv3:add(cudnn.SpatialBatchNormalization(config[2][2],1e-3))
   conv3:add(cudnn.ReLU(true))
   concat:add(conv3)

   local conv3xx = nn.Sequential()
   conv3xx:add(cudnn.Spatial_Weight_CenteredBN(  input_size, config[3][1],1,1,1,1)):add(cudnn.ReLU(true))
   conv3xx:add(cudnn.SpatialBatchNormalization(config[3][1],1e-3))
   conv3xx:add(cudnn.ReLU(true))
   conv3xx:add(cudnn.Spatial_Weight_CenteredBN(config[3][1], config[3][2],3,3,1,1,1,1)):add(cudnn.ReLU(true))
   conv3xx:add(cudnn.SpatialBatchNormalization(config[3][2],1e-3))
   conv3xx:add(cudnn.ReLU(true))
   conv3xx:add(cudnn.Spatial_Weight_CenteredBN(config[3][2], config[3][2],3,3,1,1,1,1)):add(cudnn.ReLU(true))
   conv3xx:add(cudnn.SpatialBatchNormalization(config[3][2],1e-3))
   conv3xx:add(cudnn.ReLU(true))
   concat:add(conv3xx)
   ...
   if config[4][2] ~= 0 then
      pool:add(cudnn.Spatial_Weight_CenteredBN(input_size, config[4][2],1,1,1,1)):add(cudnn.ReLU(true))
...
function createModel(opt)
   ...
   features:add(cudnn.SpatialBatchNormalization(64,1e-3))
    features:add(cudnn.ReLU(true))
   features:add(cudnn.Spatial_Weight_CenteredBN(64,192,3,3,1,1,1,1))
...
--   aux_classifier:add(cudnn.Spatial_Weight_CenteredBN(576,128,1,1,1,1))
